...
[role="_abstract"]
{product-title} requires an Amazon S3 bucket with permissions configured to store billing reports.
...
====
+
. In the {platform} platform, open the *Sources* menu (https://cloud.redhat.com/settings/sources/) to begin adding an AWS source to {product-title}:
