...
|bucket_name|Destination Amazon S3 bucket name|'cost-customer-sample-bucket'|The bucket must be configured to allow {product-title} to write files to it.
|====

.Example request

The following example requests data for the entire month of September 2019, to be written to the _cost-customer-sample-bucket_ AWS S3 bucket:
...
http localhost:8000/api/cost-management/v1/dataexportrequests/ \
    ...
    end_date='2019-10-01' \
    bucket_name='cost-customer-sample-bucket'
----
