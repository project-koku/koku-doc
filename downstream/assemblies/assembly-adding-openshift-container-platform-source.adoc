...
The community `cost-mgt-operator` supported in OpenShift Container Platform 4.3 and 4.4 is deprecated. Though this cost operator functions with cost management in OpenShift Container Platform 4.5, it is no longer supported. For installing this deprecated operator, see xref:assembly_adding_ocp_sources[].
